...
MediaObjectDecoder::MediaObjectDecoder(MediaObject* initSegment, MediaObject* mediaSegment, IMediaObjectDecoderObserver* observer, MultimediaManager* manager) :
	observer(observer),
...
bool    MediaObjectDecoder::Start() {
	...
	double frameRate = (double)this->decoder->streamconfigs.front().stream->r_frame_rate.num / (double)this->decoder->streamconfigs.front().stream->r_frame_rate.den;
	EnterCriticalSection(&this->manager->monitorMutex);
	this->manager->frameRate = frameRate;
	LeaveCriticalSection(&this->manager->monitorMutex);
