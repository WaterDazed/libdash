...
bool        MultimediaStream::Start(MultimediaManager* manager) {
	if (!this->StartDownload(manager))
		return false;

	return true;
}
bool        MultimediaStream::StartDownload(MultimediaManager* manager) {
	if (!dashManager->Start(manager))
		return false;
